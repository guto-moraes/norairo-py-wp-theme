...
export const List = ({ className, children }: ISidebarList) => (
  <ul
    className={cn(
      "text-xs text-secondary-700 font-medium flex flex-col gap-y-2.5 max-w-full",
      className
    )}
  >
    {children}
...
export const ListItem = ({ className, label, value, children }: ISidebarListItem) => (
  <li
    className={cn(
      "rounded-sm shadow-sm bg-white py-1.5 px-2.5",
      className
    )}
  >
    {label && label} {value && <strong className="text-secondary-800">{value}</strong>}{" "}
    {children && children}
  </li>
);
...
export const ExternalLink = ({ className, url, title, label }: IExternalLink) => (
  <Links
    ...
    title={title}
    className={cn(
      "flex gap-x-1 text-sky-600 hover:text-priamry-400 font-bold transition-colors duration-300",
      className
    )}
    target="_blank"
  >
...
export const Title = ({ title }: { title: string }) => (
  <h2 className="text-lg text-primary-600 font-bold">{title}</h2>
);
...
export const BoxText = ({ children }: { children: React.ReactNode }) => (
  <div
    className={cn(
      ...
      "hyphens-auto [&_p:not(:last-of-type)]:mb-2 max-h-70 overflow-y-auto",
